...
def ensure_save_dirs(base_path: str) -> None:
...
def count_files_in_directory(directory_path: str) -> Tuple[int, int]:
...
def open_video_writers(directory_path: str, fps: float, size: tuple) -> tuple:
...
def open_serial_log(directory_path: str) -> tuple:
    ensure_save_dirs(directory_path)
